...
function buildAssignment(missions, soldiers, attendanceToday, missionHistory = {}, fullAtt = {}, pinnedAssignments = {}) {
 ...
 function runPairedStrategy() {
  ...
  const _dbg = []; const _d = s => _dbg.push(s);
  const groupNums = Object.keys(tgMap).map(Number).sort((a, b) => b - a);
  _d(`groups: ${groupNums.join(',')}`);
  for (const gn of groupNums) {
   ...
   _d(`G${gn}: days=${days.join(',')}, seats/day: ${days.map(d=>`D${d}=${seatsPerDay[d].length}(${seatsPerDay[d].map(s=>s.type[0]+':'+s.missionName.substring(0,4)).join(',')})`).join(' | ')}`);
   const prevHist = {};
   const grpSoldiers = [];
   ...
   {!currentDep ? (
    ...
    <div style={S.workspace}>
     ...
     {['soldiers','missions','attendance','assignment','users'].includes(activeTab) && (
      <div style={{display:'flex',justifyContent:'flex-start',padding:'8px 0 12px'}}>
       <button onClick={forceSaveToCloud} disabled={cloudSyncStatus==='saving'}
        style={{padding:'8px 22px',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer',
         border:`1.5px solid ${cloudSyncStatus==='ok'?'#4ade80':cloudSyncStatus==='err'?'#f87171':'#3b82f6'}`,
 ...
 async function runForce() {
  ...
  try {
   ...
   if (out.feasible) {
    ...
    setForceViolations(V);
    notify(V.length ? `שובץ עם ${V.length} חריגות מהחוקים` : 'שובץ ללא חריגות', V.length?'warn':'success');
   } else {
    notify('גם מילוי כפוי לא הצליח — אין מספיק חיילים פיזית', 'error');
   }
  } catch(e) {
   console.error('Force error:', e); notify('שגיאת מילוי כפוי', 'error');
  }
  setIsGenerating(false);
 }
 /* שיבוץ ישן — גיבוי */
